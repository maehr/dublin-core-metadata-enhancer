...
class TestMetadataEnhancer(unittest.TestCase):
    ...
    def test_build_prompt(self):
        ...
        self.assertIn("WCAG 2.2-konform", prompt)
    ...
    def test_get_image_bytes(self, mock_image_open, mock_get):
        ...
        # Mock PIL Image
        mock_img = Mock()
        mock_img.size = (400, 300)  # Small image, no resizing needed
        mock_img.convert.return_value = mock_img
        mock_img.save = Mock()
        mock_image_open.return_value = mock_img
        
        # Mock BytesIO for the result
        with patch('metadata_enhancer.BytesIO') as mock_bytesio:
            mock_buf = Mock()
            mock_buf.read.return_value = b"processed_image_data"
            mock_bytesio.return_value = mock_buf
            
            result = self.enhancer.get_image_bytes("https://example.com/test.jpg")
            
            self.assertEqual(result, b"processed_image_data")
